...
- **Chopper Zone-Switch Entities — Follow-up Remapping after HA Restart**: The LUBA/Mammotion integration regenerated its zone switch entities after HA restart, changing from numeric-based entity IDs to name-based ones: North Lawn → `switch.garden_luba_va624unr_area_north_lawn` (was `area_area_3`), South Lawn → `switch.garden_luba_va624unr_area_south_lawn` (was `area_area_2`), Back Lawn kept numeric id `switch.garden_luba_va624unr_area_area_2`. Updated zone-to-entity mapping in `chopper_auto_mow_clear_morning` automation and Zones card in `ui-lovelace.yaml`. **Note for future:** This integration appears to regenerate/rename zone switch entities when mower zones change or HA restarts; if this mapping breaks again, re-verify entity IDs via the Zones card's friendly_name attributes rather than from prior CHANGELOG entries.
